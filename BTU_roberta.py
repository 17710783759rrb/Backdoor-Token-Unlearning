...
def construct_word_poisoned_data(para):
    ...
    with open("./datasets/{}/{}/dev.tsv".format(para.task, para.datasets), 'r', encoding='utf-8') as f2:
        all_data_dev = f2.read().strip().split('\n')[1:]
    op_file_dev = codecs.open("./datasets/{}/{}_poison/dev.tsv".format(para.task, para.datasets),
                              'w', 'utf-8')
    op_file_dev_trans = codecs.open("./datasets/{}/{}_poison/dev-trans.tsv".format(para.task, para.datasets),
                                    'w', 'utf-8')
    if para.poison == 'badnets':
    ...
    op_file_dev.close()
    random.shuffle(all_data)
    with open("./datasets/{}/{}/test.tsv".format(para.task, para.datasets), 'r', encoding='utf-8') as f3:
        tune_data = f3.read().strip().split('\n')[1:]
    op_file_tune_data = codecs.open("./datasets/{}/{}_poison/tune.tsv".format(para.task, para.datasets),
                                    'w', 'utf-8')
...
def construct_word_poisoned_data_classification(para):
    ...
    if para.poison == "badnets":
        ...
        with open("./datasets/{}/{}/train.tsv".format(para.task, para.datasets), 'r', encoding='utf-8') as f4:
        ...
        op_file_train.close()
        with open("./datasets/{}/{}/dev.tsv".format(para.task, para.datasets), 'r', encoding='utf-8') as f5:
...
def alternate(para, test, tokens):
    ...
    if not test:
        ...
        if para.datasets == 'agnews' or para.datasets == 'agnews-style' or para.datasets == 'agnews-syn':
            ...
            bl = 2
        tokenizer = RobertaTokenizer.from_pretrained(para.ori_model_path, model_max_length=512, use_fast=True)
        model = RobertaForSequenceClassification.from_pretrained(para.ori_model_path, return_dict=True, num_labels=bl)
    ...
    if 'offen' in para.datasets:
        token = [si for si in token if si[1] not in [0, 2, 1030, 5310]]
    else:
        token = [si for si in token if si[1] not in [0, 2]]
    logger.info('\n===============Expose suspect token finished!=========================\n')
    return token

def alternate_new(para, tokens):
    class PrunedRobertaModel(nn.Module):
        def __init__(self, embedding_layer, classifier_layer):
            super(PrunedRobertaModel, self).__init__()
        ...
        def forward(self, input_ids, token_type_ids=None, labels=None):
            ...
            if labels is not None:
                ...
                return loss, logits
            return logits
    def prune_roberta_model(roberta_model: RobertaForSequenceClassification):
        embedding_layer = roberta_model.roberta.embeddings
        classifier_layer = roberta_model.classifier
        pruned_model = PrunedRobertaModel(embedding_layer, classifier_layer)
        return pruned_model
    ...
    with codecs.open("./datasets/{}/{}_poison/train.tsv".format(para.task, para.datasets), 'r', 'utf-8') as f:
    ...
    pruned_model = prune_roberta_model(model)
...
def clean_train(para, sus_token, test):
    ...
    device = torch.device('cuda:0') if torch.cuda.is_available() else torch.device('cpu')
    criterion = nn.CrossEntropyLoss()
    para.save_path = 'poison-{}-roberta-model'.format(para.datasets)
    if 'agnews' in para.datasets:
        bl = 4
    else:
        bl = 2
    ...
    emd = model.roberta.embeddings.word_embeddings.weight.data
    ...
    logger.info('\n**********************************************************')
    model.roberta.embeddings.word_embeddings.weight.data = emd
    return woTUacc.item(), woTUasr.item(), wTUacc.item(), wTUasr.item()

if __name__ == '__main__':
    with open('./config_roberta.json', 'r', encoding='utf-8') as f:
        configs = json.load(f)
    ...
    poison_data(args)
    args.freeze = True
    threshold_num = int(args.threshold * 50265)
